...
export default function Page () {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const res = await fetch('/api/ks/list?app=ttyd');
            const json = await res.json();
            if (json.status == "Running" && loading == false) { 
                setStatus(json.status);
                setLink("http://"+session.user.name+".ttyd.app.ksc.re.kr")            }
        }
        fetchData()
    }, [session])
    ...
    const click_create = async () => {
        const res = await fetch('/api/ks/create?app=ttyd')
        const json = await res.json()
        setStatus("creating")
        click_connect()
    }
    const click_delete = async () => {
        const res = await fetch('/api/ks/delete?app=ttyd')
        const json = await res.json()
        setLink("")
        setStatus("-")
        clearTimeout(timer);
